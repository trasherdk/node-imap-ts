...
var RESPONSES = [
	...
	['* NAMESPACE (("" "/")) NIL NIL', "A2 OK Success", ""].join(CRLF),
	['* LIST (\\Noselect) "/" "/"', "A3 OK Success", ""].join(CRLF),
	[
	...
	["* BYE LOGOUT Requested", "A7 OK good day (Success)", ""].join(CRLF),
];
var EXPECTED = [
	"A0 CAPABILITY",
	'A1 LOGIN "foo" "bar"',
		...
		if (buf.indexOf(CRLF) > -1) {
			...
			lines.forEach(function(l) {
				...
				if (l === "DONE") {
					...
					assert(
						RESPONSES[++res],
						"No response for client request: " + l,
					);
