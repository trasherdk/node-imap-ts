import { parseExpr, ParsedExpression } from "./common";
import { parseFetchEnvelope } from "./fetch";

export function parseBodyStructure(
	cur: string | ParsedExpression,
	literals?: string[],
) {
	const result = Array.isArray(cur) ? cur : parseExpr(cur, literals);
	if (result.length) {
		return _parseBodyStructure(result, literals, "", 1);
	}
	return [];
}

function _parseBodyStructure(cur, literals, prefix, partID) {
	let ret = [];
	let i;
	let len;
	let part;
	...
	if (Array.isArray(cur[0])) {
		...
		if (part.type === "message" && part.subtype === "rfc822") {
			...
			// body
			if (partLen > next && Array.isArray(cur[next])) {
				part.body = _parseBodyStructure(cur[next], literals, prefix, 1);
			} else {
				part.body = null;
			}
			++next;
